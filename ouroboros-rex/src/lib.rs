...
use rex_type_system::types::{Type as RexType, ADT, ADTVariant};

// Note: We cannot use From/Into here as these only work if the types
// they are implemented on are defined in the same crate

pub fn ouroboros_to_rex(our_type: &OuroborosType) -> RexType {
    match our_type {
        OuroborosType::Unit => RexType::Tuple(vec![]),
        OuroborosType::Bool => RexType::Bool,
        OuroborosType::I8 => RexType::Int,
        OuroborosType::I16 => RexType::Int,
        OuroborosType::I32 => RexType::Int,
        OuroborosType::I64 => RexType::Int,
        OuroborosType::I128 => panic!("Unsupported ouroboros type: I128"),
        OuroborosType::U8 => RexType::Uint,
        OuroborosType::U16 => RexType::Uint,
        OuroborosType::U32 => RexType::Uint,
        OuroborosType::U64 => RexType::Uint,
        OuroborosType::U128 => panic!("Unsupported ouroboros type: U128"),
        OuroborosType::F32 => RexType::Float,
        OuroborosType::F64 => RexType::Float,
        OuroborosType::String => RexType::String,
        OuroborosType::Array(a) => RexType::List(Box::new(ouroboros_to_rex(&a.t))),
        OuroborosType::Record(r) => {
            ...
            RexType::ADT(ADT {
                ...
                variants: vec![ADTVariant {
                    name: r.n.clone(),
                    t: Some(Box::new(fields_to_rex(&r.fields))),
                    docs: None,
                    t_docs: t_docs,
                }],
        ...
        OuroborosType::Func(func) => RexType::Arrow(
            Box::new(ouroboros_to_rex(&*func.a)),
            Box::new(ouroboros_to_rex(&*func.b))),
        ...
        OuroborosType::Fallible(f) => RexType::Result(
            Box::new(ouroboros_to_rex(&f.ok)),
            Box::new(ouroboros_to_rex(&f.err))),
        OuroborosType::Optional(o) => RexType::Option(Box::new(ouroboros_to_rex(&o.t))),
        OuroborosType::Union(u) => RexType::ADT(ADT {
            name: u.n.clone(),
            variants: u.variants
                .iter()
                .map(|v| ADTVariant {
                    ...
                    t: match &v.fields {
                        Some(Fields::Named(fields)) => {
                            Some(Box::new(named_fields_to_rex(fields)))
                        }
                        Some(Fields::Unnamed(fields)) => {
                            match fields.fields.len() {
                                0 => None,
                                1 => Some(Box::new(ouroboros_to_rex(&fields.fields[0].t))),
                                _ => Some(Box::new(unnamed_fields_to_rex(fields))),
                            }
                        }
                        None => None,
                    },
        ...
        OuroborosType::Ptr(ptr) => {
            RexType::ADT(ADT {
                ...
                variants: vec![
                    ADTVariant {
                        name: "Ptr".to_string(),
                        t: Some(Box::new(ouroboros_to_rex(&*ptr.t))),
                        docs: None,
                        t_docs: None,
                    }
                ],
        ...
        OuroborosType::Alias(alias) => {
            let rex_type: RexType = ouroboros_to_rex(&*alias.t);
            RexType::ADT(ADT {
                name: alias.n.clone(),
                variants: vec![
                    ADTVariant {
                        name: alias.n.clone(),
                        t: Some(Box::new(rex_type)),
                        docs: None,
                        t_docs: None,
                    }
                ],
...
fn fields_to_rex(fields: &Fields) -> RexType {
    match fields {
...
fn rex_record_to_ouroboros(
    ...
    entries: &BTreeMap<String, RexType>,
) -> OuroborosType {
...
pub mod test {
    ...
    use rex_proc_macro::*;
    use rex_type_system::types::{ToType};
    use ouroboros::type_info::TypeInfo;
    use ouroboros::ptr::Ptr as OuroborosPtr;
    ...
    fn test_tuple1() {
        ...
        assert_eq!(rex_type, RexType::Tuple(vec![RexType::Uint]));
    ...
    fn test_tuple2() {
        ...
        let rex_type = ouroboros_to_rex(&our_type);
        assert_eq!(rex_type, RexType::Tuple(vec![RexType::Uint, RexType::String]));
        assert_eq!(rex_to_ouroboros(&rex_type), our_type);
    }
    ...
    fn test_func() {
        ...
        let rex_type = RexType::Arrow(
            Box::new(RexType::Uint),
            Box::new(RexType::String));
