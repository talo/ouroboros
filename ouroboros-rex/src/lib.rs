...
pub fn ouroboros_to_rex(our_type: &OuroborosType) -> Result<Arc<RexType>, String> {
    match our_type {
        ...
        OuroborosType::Array(a) => Ok(Arc::new(RexType::List(ouroboros_to_rex(&a.t)?))),
        ...
        OuroborosType::Optional(o) => Ok(Arc::new(RexType::Option(ouroboros_to_rex(&o.t)?))),
...
fn fields_to_rex(fields: &Fields) -> Result<Arc<RexType>, String> {
    match fields {
...
fn named_fields_to_rex(fields: &NamedFields) -> Result<Arc<RexType>, String> {
...
fn unnamed_fields_to_rex(fields: &UnnamedFields) -> Result<Arc<RexType>, String> {
...
pub mod test {
    ...
    fn test_unit() {
        assert_eq!(
            *ouroboros_to_rex(&OuroborosType::Unit).unwrap(),
            RexType::Tuple(vec![])
        );
        assert_eq!(
    ...
    fn test_tuple1() {
        ...
        let rex_type = ouroboros_to_rex(&our_type).unwrap();
    ...
    fn test_enum_int() {
        ...
        assert_eq!(
            *ouroboros_to_rex(&int::Foo::t()).unwrap(),
            int::Foo::to_type()
        );
        assert_eq!(
    ...
    fn test_numeric_roundtrip() {
        ...
        assert_eq!(
            rex_to_ouroboros(&ouroboros_to_rex(&different_sizes::Foo::t()).unwrap()).unwrap(),
            all_64::Foo::t()
        );

        assert_eq!(
            *ouroboros_to_rex(&different_sizes::Foo::t()).unwrap(),
            all_64::Foo::to_type()
        );
    }
    ...
    fn test_alias() {
        ...
        assert_eq!(*ouroboros_to_rex(&our_type).unwrap(), rex_type);
        assert_eq!(rex_to_ouroboros(&rex_type).unwrap(), our_type);
    }
