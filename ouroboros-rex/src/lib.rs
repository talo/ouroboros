...
use rex::{ADT, ADTVariant, EnumPatch, JsonOptions, Type as RexType};
use std::{
...
pub fn ouroboros_to_rex(
    ...
    match our_type {
        ...
        OuroborosType::Bool => Ok(Arc::new(RexType::Bool)),
        OuroborosType::I8 => Ok(Arc::new(RexType::Int)),
        OuroborosType::I16 => Ok(Arc::new(RexType::Int)),
        OuroborosType::I32 => Ok(Arc::new(RexType::Int)),
        OuroborosType::I64 => Ok(Arc::new(RexType::Int)),
        OuroborosType::I128 => Err("Unsupported ouroboros type: I128".to_string()),
        OuroborosType::U8 => Ok(Arc::new(RexType::Uint)),
        OuroborosType::U16 => Ok(Arc::new(RexType::Uint)),
        OuroborosType::U32 => Ok(Arc::new(RexType::Uint)),
        OuroborosType::U64 => Ok(Arc::new(RexType::Uint)),
        OuroborosType::U128 => Err("Unsupported ouroboros type: U128".to_string()),
        OuroborosType::F32 => Ok(Arc::new(RexType::Float)),
        OuroborosType::F64 => Ok(Arc::new(RexType::Float)),
        OuroborosType::String => Ok(Arc::new(RexType::String)),
        OuroborosType::Array(a) => Ok(Arc::new(RexType::List(ouroboros_to_rex(&a.t, opts)?))),
        ...
        OuroborosType::Func(func) => Ok(Arc::new(RexType::Arrow(
            ...
            Ok(Arc::new(RexType::ADT(ADT {
                ...
                variants: e
                    ...
                    .map(|v| ADTVariant {
                        ...
                        docs: None,
                        t_docs: None,
                        discriminant: get_discriminant(&e, &v, patches),
                    })
                    .collect::<Vec<_>>(),
                docs: None, // Not generated by #[derive(TypeInfo)]
            })))
        }
        OuroborosType::Fallible(f) => Ok(Arc::new(RexType::Result(
            ouroboros_to_rex(&f.ok, opts)?,
            ouroboros_to_rex(&f.err, opts)?,
        ))),
        OuroborosType::Optional(o) => Ok(Arc::new(RexType::Option(ouroboros_to_rex(&o.t, opts)?))),
...
fn get_discriminant(e: &Enum, v: &EnumVariant, patches: Option<&Vec<EnumPatch>>) -> Option<i64> {
    ...
    if let Some(patches) = patches {
        for p in patches.iter() {
            if p.enum_name == v.n {
                println!(
                    "******** {}: patching variant {} with discriminant {}",
                    e.n, v.n, p.discriminant
                );
...
pub fn rex_to_ouroboros(rex_type: &RexType) -> Result<OuroborosType, String> {
    match rex_type {
        ...
        RexType::Result(ok, err) => Ok(OuroborosType::Fallible(Fallible {
            ok: Box::new(rex_to_ouroboros(ok)?),
            err: Box::new(rex_to_ouroboros(err)?),
        })),
        RexType::Option(t) => Ok(OuroborosType::Optional(Optional {
            t: Box::new(rex_to_ouroboros(t)?),
        })),
        RexType::Promise(_) => Err("Unsupported Rex type: Promise".to_string()),
        RexType::List(t) => Ok(OuroborosType::Array(Array::new(rex_to_ouroboros(t)?))),
        ...
        RexType::Bool => Ok(OuroborosType::Bool),
        RexType::Uint => Ok(OuroborosType::U64),
        RexType::Int => Ok(OuroborosType::I64),
        RexType::Float => Ok(OuroborosType::F64),
        RexType::String => Ok(OuroborosType::String),
        RexType::Uuid => Err("Unsupported Rex type: Uuid".to_string()),
        RexType::DateTime => Err("Unsupported Rex type: DateTime".to_string()),
    }
}
