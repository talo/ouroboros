...
// Handle multiple arguments where every argument is stand-alone. Supports up to
// 32 arguments.
...
// Handle multiple arguments where every argument is a pair of arguments.
// Supports up to 32 arguments.
...
#define OUROBOROS_TYPE_NAME(X1) #X1,
...
#define OUROBOROS_ENUM_JSON_TO(Y1) \
    case Y1:                       \
        nlohmann_json_j = #Y1;     \
        break;
#define OUROBOROS_ENUM_JSON_FROM(Y1)         \
    if (nlohmann_json_s == std::string(#Y1)) \
    {                                        \
        nlohmann_json_t = Y1;                \
        return;                              \
    }

#define OUROBOROS_STRUCT_JSON_TO(X1, Y1) nlohmann_json_j[#Y1] = nlohmann_json_t.Y1;
#define OUROBOROS_STRUCT_JSON_FROM(X1, Y1) nlohmann_json_j.at(#Y1).get_to(nlohmann_json_t.Y1);

#define OUROBOROS_STRUCT(Type, ...)                                                                                                                                         \
    struct Type                                                                                                                                                             \
    {                                                                                                                                                                       \
        OUROBOROS_EXPAND(OUROBOROS_PASTE2(OUROBOROS_FIELD, __VA_ARGS__))                                                                                                    \
        friend void to_json(nlohmann::json &nlohmann_json_j, const Type &nlohmann_json_t) { OUROBOROS_EXPAND(OUROBOROS_PASTE2(OUROBOROS_STRUCT_JSON_TO, __VA_ARGS__)) }     \
        friend void from_json(const nlohmann::json &nlohmann_json_j, Type &nlohmann_json_t) { OUROBOROS_EXPAND(OUROBOROS_PASTE2(OUROBOROS_STRUCT_JSON_FROM, __VA_ARGS__)) } \
    };                                                                                                                                                                      \
    template <>                                                                                                                                                             \
    struct ouroboros::TypeInfo<Type>                                                                                                                                        \
    {                                                                                                                                                                       \
        static nlohmann::json type_info()                                                                                                                                   \
        {                                                                                                                                                                   \
            return nlohmann::json{{"k", "record"}, {"t", nlohmann::json{OUROBOROS_EXPAND(OUROBOROS_PASTE2(OUROBOROS_TYPE_INFO, __VA_ARGS__))}}};                            \
        }                                                                                                                                                                   \
    };

#define OUROBOROS_ENUM(Type, ...)                                                                                                  \
    enum Type                                                                                                                      \
    {                                                                                                                              \
    ...
    {                                                                                                                              \
        switch (nlohmann_json_t)                                                                                                   \
        {                                                                                                                          \
            OUROBOROS_EXPAND(OUROBOROS_PASTE1(OUROBOROS_ENUM_JSON_TO, __VA_ARGS__))                                                \
        }                                                                                                                          \
    }                                                                                                                              \
    void from_json(const nlohmann::json &nlohmann_json_j, Type &nlohmann_json_t)                                                   \
    {                                                                                                                              \
        std::string nlohmann_json_s = nlohmann_json_j;                                                                             \
        OUROBOROS_EXPAND(OUROBOROS_PASTE1(OUROBOROS_ENUM_JSON_FROM, __VA_ARGS__))                                                  \
    }
